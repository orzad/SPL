...
public abstract class MicroService implements Runnable {
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        this.callbackHashMap.put(type,callback);
    }
     ...
     */
    @Override
    public final void run() {     //need to copy that to ors section
        initialize();
        while (!terminated) {
            try {
                Message m = MessageBusImpl.getInstance().awaitMessage(this);
                if (m.getClass() == TickBroadcast.class || m.getClass() == FinishedTestBroadcast.class || m.getClass() == FinishedTrainBroadcast.class ||
                        m.getClass() == PublishConferenceBroadcast.class || m.getClass() == TerminateBroadcast.class) {
                       callbackHashMap.get(m.getClass()).call(m);
                }
                else {
                    events.add((Event) m);
                    if (events.size() == 1) {
                        callbackHashMap.get(m.getClass()).call(events.peek());
                    }
                }
                if (!events.isEmpty() && events.peek().getFinished()) {
                    events.remove();
                    if(events.peek()!=null) {
                        callbackHashMap.get(events.peek().getClass()).call(events.peek());
                    }
                }
